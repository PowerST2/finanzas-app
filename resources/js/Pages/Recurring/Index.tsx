...
export default function Index({ rules, wallets, categories }: any) {
    const firstExpense = categories.find((c: any) => c.type === 'expense');
    const form = useForm({ wallet_id: wallets[0]?.id || '', category_id: firstExpense?.id || '', type: 'expense', frequency: 'monthly', amount: '', description: '', next_at: nowLocal() });
        ...
        <AuthenticatedLayout header={<h2 className="text-xl font-semibold text-gray-800">Movimientos recurrentes</h2>}>
            ...
            <div className="mx-auto grid max-w-5xl gap-4 px-4 py-6 lg:grid-cols-[.8fr_1.2fr]">
                <form onSubmit={(e) => { e.preventDefault(); form.post(route('recurring.store'), { onSuccess: () => form.reset('amount', 'description') }); }} className="space-y-3 rounded-lg bg-white p-4 shadow-sm">
                    ...
                    <select className="w-full rounded-lg border-gray-300" value={form.data.type} onChange={(e) => {
                        const type = e.target.value;
                        form.setData({ ...form.data, type, category_id: categories.find((c: any) => c.type === type)?.id || '' });
                    }}><option value="expense">Egreso</option><option value="income">Ingreso</option></select>
                    ...
                    <select className="w-full rounded-lg border-gray-300" value={form.data.category_id} onChange={(e) => form.setData('category_id', e.target.value)}>{filtered.map((c: any) => <option key={c.id} value={c.id}>{c.name}</option>)}</select>

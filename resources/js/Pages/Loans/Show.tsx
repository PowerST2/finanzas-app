        ...
        <AuthenticatedLayout header={<h2 className="text-xl font-semibold text-gray-800">{loan.name}</h2>}>
            ...
            <div className="mx-auto grid max-w-5xl gap-4 px-4 py-6 lg:grid-cols-2">
                <section className="rounded-lg bg-white p-4 shadow-sm">
                    ...
                    <div className="mt-2 text-sm text-gray-500">{loanKind(loan.kind)} · Original {money(loan.principal_amount)} · Estado {statusLabel(loan.status)}</div>
                ...
                {loan.status === 'active' && <form onSubmit={(e) => { e.preventDefault(); form.post(route('loans.pay', loan.id)); }} className="space-y-4 rounded-lg bg-white p-4 shadow-sm">
...
function statusLabel(status: string) { return status === 'paid' ? 'pagado' : status === 'suspended' ? 'suspendido' : 'activo'; }
